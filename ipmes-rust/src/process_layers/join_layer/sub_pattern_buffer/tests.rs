...
fn test_try_merge_edges1() {
    ...
    assert!(sub_pattern_buffer.try_merge_match_events(&match_edge1, &match_edge2).is_none());
}
...
fn gen_match_edges<'p>(pattern_edges: &'p Vec<Event>, input_edge_data: &Vec<(u64, u64)>) -> Vec<MatchEvent<'p>> {
...
fn cmp_match_edge(edge1: &MatchEvent, edge2: &MatchEvent) -> bool {
    ...
    else if edge1.input_event.id != edge2.input_event.id { return false }
    true
}

/// Below codes can be used to test "try_merge_entities" for large testcases, but there might be some bugs.
...
//     if let Some(merged) = try_merge_entities(&a, &b) {
//         assert_ne!(ans, None);
